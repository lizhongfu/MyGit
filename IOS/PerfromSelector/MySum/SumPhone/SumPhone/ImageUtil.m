...
// 返回一个使用RGBA通道的位图上下文 
static CGContextRef CreateRGBABitmapContext_1 (CGImageRef inImage) 
{
	CGContextRef context = NULL; 
	CGColorSpaceRef colorSpace; 
...
static unsigned char *RequestImagePixelData_1(UIImage *inImage) 
{
	CGImageRef img = [inImage CGImage]; 
	CGSize size = [inImage size];
    //使用上面的函数创建上下文
	CGContextRef cgctx = CreateRGBABitmapContext_1(img); 
...
{
unsigned char *imgPixel = RequestImagePixelData_1(inImage);
CGImageRef inImageRef = [inImage CGImage];
GLuint w = CGImageGetWidth(inImageRef);
GLuint h = CGImageGetHeight(inImageRef);
